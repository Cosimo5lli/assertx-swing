...
import org.assertx.swing.assertXSwing.AXSNamed
...
class AssertXSwingValidator extends AbstractAssertXSwingValidator {
	...
	public static val SAME_NAME = ISSUE_CODE_PREFIX + 'SameName'
	...
	def void checkMembersNamesAreUnique(AXSTestCase tc) {
		val filtered = tc.members.filter(AXSNamed).filterInMultimap(AXSTestMethod, AXSMatcher)

		filtered.get(AXSTestMethod).checkDuplicatesAndCreateDiagnostic(
			Severity.WARNING,
			'test method',
			'test methods should use different and explanatory names'
		)
		filtered.get(AXSMatcher).checkDuplicatesAndCreateDiagnostic(
			Severity.ERROR,
			'matcher',
			'matchers with same name are not permitted within the same test case'
		)
	}

	def private void checkDuplicatesAndCreateDiagnostic(HashMultimap<String, ? extends AXSNamed> named,
		Severity severity, String type, String secondPart) {

		for (e : named.asMap.entrySet) {
			val collisions = e.value
			if (collisions.size > 1) {
				for (c : collisions) {
					addAppropriateSeverityDiagnostic(
						severity,
						'''Duplicate «type» '«e.key»': «secondPart»''',
						c,
						AssertXSwingPackage.eINSTANCE.AXSNamed_Name,
	...
	def void checkTestedClassMustBeSubclassOfJFrameOrJDialog(AXSTestCase tc) {
		val clazz = tc.typeRef.toLightweightTypeReference
		if (!(clazz.isSubtypeOf(JFrame) || clazz.isSubtypeOf(JDialog)))
			error(
				...
				AssertXSwingPackage.eINSTANCE.AXSDefinable_TypeRef, UNTESTABLE_TYPE)
	}
	...
	def void checkAutogeneratedMethodsAccess(XAbstractFeatureCall featureCall) {
		...
		val autogeneratedNames = testCase.generatedMethodsNames
	...
	def void checkMatchingTypeInMatcher(AXSMatcher m) {
		val type = m.typeRef.toLightweightTypeReference
		if (!type.isSubtypeOf(Component)) {
			error(
				'Unmatchable type: "' + m.typeRef.qualifiedName +
	...
	@Check
	def void checkDefinableNamesAreUnique(AXSFile file) {
		val filtered = file.definitions.filterInMultimap(AXSTestCase, AXSMatcher)

		filtered.get(AXSTestCase).checkDuplicatesAndCreateDiagnostic(
			Severity.ERROR,
			'test case',
			'test cases with same name are not permitted within the same file'
		)

		filtered.get(AXSMatcher).checkDuplicatesAndCreateDiagnostic(
			Severity.ERROR,
			'matcher',
			'matchers with same name are not permitted within the same file'
		)
	}
	...
	private def <T1 extends AXSNamed, T2 extends AXSNamed> Map<Class<? extends AXSNamed>, HashMultimap<String, ? extends AXSNamed>> filterInMultimap(
		...
		for (element : list) {
			if (type1.isInstance(element)) {
				firstTypes.put(element.name, (element as T1))
			} else if (type2.isInstance(element)) {
