...
class AssertXSwingStaticExtensions {
	...
	def static getCheckedVariableName(AXSMatcherRef ref) {
		ref.variableName ?: ref.reference.name
	}

	def static getActualVariableName(AXSMatcherRef ref) {
		'_' + ref.checkedVariableName
	}
	...
	def static List<String> getGeneratedMethodsNames(AXSTestCase tc) {
		val list = getDefaultGeneratedMethodsNames(tc)
		if(tc === null) return list
		list.addMatcherGeneratedMethodsNames(tc)
		return list
	}

	def static private getDefaultGeneratedMethodsNames(AXSTestCase tc) {
	...
	def static private addMatcherGeneratedMethodsNames(List<String> list, AXSTestCase tc) {
		val mappings = tc.camelCaseMethodsNamesMappings

		for (test : tc.tests.filter[matchers?.size > 0]) {
			list += mappings.getAutogeneratedMethodNameForMatcher(test)
		}
	}
}
