...
class AssertXSwingValidationTest {
	...
	def void testWarningOnMethodWithSameName() {
		...
		input.parse => [
			assertWarning(
				...
				AssertXSwingValidator.SAME_NAME_METHODS,
				input.indexOf(name),
				name.length,
	...
	def void testMatchersMatchingTypeComplyWithGenericTypeMatcherTypeParameterBound() {
		...
		input.parse.assertError(
			...
			input.indexOf(type),
			type.length,
			'Unmatchable type: the type "java.lang.Object" is not suitable for a matcher definition, only subtypes of "java.awt.Component" can be matched'
		)
	}
	
	@Test
	def void testMatcherGeneratedMethodsCannotBeAccessed(){
		'''
		testing javax.swing.JFrame
		
		aName match javax.swing.JLabel {
			true
		}
		
		test 'a test' using aName {
			_generated_aTest
			this._generated_aTest
		}
		'''.parse => [
			assertAutogeneratedMethod('_generated_aTest', XbasePackage.eINSTANCE.XFeatureCall)
			assertAutogeneratedMethod('_generated_aTest', XbasePackage.eINSTANCE.XMemberFeatureCall)
		]
	}
